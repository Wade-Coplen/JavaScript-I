...
// ==== Challenge 1 ====
// // The dealer can't recall the information for a car with an id of 33 on his lot. Help the dealer find out which car has an id of 33 by logging the car's year,
...
let carModels = inventory.sort(function(make, model) {
  return make.car_make + model.car_model});
let carModelsSorted = [];
...
let oldCars = carYears.filter(function(carYear) {
  if(carYear <= 2000) {
    return carYear;
  }
});
...
let BMWAndAudi = inventory.filter(function(inv) {
